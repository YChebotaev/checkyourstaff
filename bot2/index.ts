...
import { logger, generatePinCode, createBot, joinByPin, sendCloseMessage, sendNextQuestion, sheduleJob, startPollSession } from './lib'
...
bot.start(async (ctx, next) => {
  ...
  const { message_id } = await ctx.sendMessage('Введите пин-код', Markup.forceReply())
...
bot.command('register', async (ctx, next) => {
  ...
  await ctx.reply('Пройдите шаги по регистрации', Markup.inlineKeyboard([
    Markup.button.webApp(
      'Пройти регистрацию',
      `${process.env['WEBAPP_URL']!}/register?userId=${userSession.userId}&chatId=${ctx.chat.id}`
    )
  ]))
...
bot.command('join', async (ctx, next) => {
  ...
  if (!userSession) {
    logger.warn("User session by chat id = %s not found or deleted", ctx.chat.id)
...
bot.command(groupCreateCommandRegexp, async (ctx, next) => {
  ...
  if (!strAccountId) {
    logger.warn('Command %s was matched but accountId was not matched', ctx.command)
  ...
  if (!accountId) {
    logger.warn('Command %s was matched but accountId = "%s" was not parsed', ctx.command, strAccountId)
  ...
  const { message_id } = await ctx.reply('Теперь введите название группы:', Markup.forceReply())
...
bot.command(groupInviteCommandRegexp, async (ctx, next) => {
  const [_, strSessionGroupId] = ctx.command.match(groupInviteCommandRegexp) ?? []

  if (!strSessionGroupId) {
    logger.warn('Command %s was matched but groupId was not matched', ctx.command)
  ...
  if (!sampleGroupId) {
    logger.warn('Command %s was matched but accountId = "%s" was not parsed', ctx.command, strSessionGroupId)
  ...
  if (!sampleGroupExists) {
    logger.warn("Group with id = %s not found while processing command = %s", sampleGroupId, ctx.command)
...
bot.command(pollCreateCommandRegexp, async (ctx, next) => {
  ...
  const { message_id } = await ctx.reply('Введите название опроса:', Markup.forceReply())
...
bot.command(questionCreateCommandRegexp, async (ctx, next) => {
  ...
  if (!strPollId) {
    logger.warn('Command %s was matched but pollId was not matched', ctx.command)
  ...
  if (!pollId) {
    logger.warn('Command %s was matched but pollId = "%s" was not parsed', ctx.command, strPollId)
  ...
  const { message_id } = await ctx.reply('Введите текст вопроса:', Markup.forceReply())
...
const sheduleSessionCommandRegexp = /shedule_session_(\d+)(?:_(x|\d+))(?:_(\d+))?/

bot.command(sheduleSessionCommandRegexp, async (ctx, next) => {
  const [_, strAccountId, strMaybeSampleGroupId, strMaybePollId] = ctx.command.match(sheduleSessionCommandRegexp) ?? []

  const accountId = Number(strAccountId)
  const sampleGroupId: number | null = strMaybeSampleGroupId === 'x' ? null : Number(strMaybeSampleGroupId)
...
bot.on('message', async (ctx, next) => {
  ...
  const originMessageMeta = await messageMetaGetByChatId(ctx.chat.id, replyToMessageId)
  ...
  switch (type) {
    ...
    case 'enter-account-name': {
      ...
      await accountAdministratorCreate({ accountId, userId: userSession.userId })
...
`)
    ...
    case 'enter-question-text': {
      ...
      if (!poll) {
        logger.warn('Poll with id = %s not found or deleted', originMessageMeta.pollId)
...
`)
    ...
    case 'enter-final-feedback': {
      ...
      void sendCloseMessage(bot, originMessageMeta.responderId!)
        .catch(e => logger.error(e))
...
bot.command('start_poll_session', async (ctx, next) => {
  ...
  void startPollSession(bot, pollId, sampleGroupId)
    .catch(e => logger.error(e))
...
bot.on('callback_query', async (ctx, next) => {
  ...
  if (parsedUrl.pathname === '/sf') {
    ...
    await pollSessionUpdatePollingState(
      pollSessionId,
      pollingState => {
        return {
          ...pollingState,
          [responderId]: without(
            pollingState[responderId],
            pollQuestionId
          )
        }
      }
    ...
    if (requestTextFeedback) {
      ...
      void sendNextQuestion(bot, pollSessionId, responderId)
        .catch(e => logger.error(e))
    }
  }
