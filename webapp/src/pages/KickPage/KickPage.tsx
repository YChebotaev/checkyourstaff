...
export const KickPage: FC = () => {
  ...
  const { mutate } = useMutation({
    async mutationFn() {
      const { data } = await apiClient.post("/kickMembers", {
        sampleGroupId,
        list: rawList,
  ...
  return (
    <AppLayout>
      <Root>
        <Text>Убрать респондентов из группы «{sampleGroup!.name}»</Text>
        <InviteMemebers textareaProps={{ rows: 7 }} onChangeList={setRawList} />
        <MainButton text="Убрать" onClick={() => mutate()} />
      </Root>
    </AppLayout>
  );
};
